import { amor } from "@/utils/diversion";
import { fallo, parametro, texto } from "@/utils/respond";

/**
 * `!amor fulano` o `!amor fulano mengano`.
 *
...
export default function handler(req, res) {
  ...
  if (!a || !b) return fallo(res, 400, "❌ Uso: !amor <nombre> [otro nombre]");

  const corto = (v) => String(v).replace(/^@+/, "").slice(0, 25);
  return texto(res, amor(corto(a), corto(b)));
}
